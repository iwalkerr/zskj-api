...
import (
	...
	configService "xframe/backend/core/system/config/service"
	dictService "xframe/backend/core/system/dict/service"
...
func (s *Server) Template(path string) *Server {
	s.Handler.HTMLRender = gintemplate.New(gintemplate.TemplateConfig{
		...
		Extension: ".html",
		Partials: []string{
			"core/system/layout/header",
			"core/system/layout/lefter",
			"core/system/layout/tab",
			"core/system/menu/icon",
			"core/header",
			"core/footer",
		},
		Funcs: template.FuncMap{
			"OssUrl":      configService.GetOssUrl,
			"DictSelect":  dictService.GetDictSelect,
			"GetDictData": dictService.GetDictData,
			"DictRadio":   dictService.GetDictRadio,
			"DictLabel":   dictService.GetDictLabel,
			"HasPermi":    indexService.HasPermi,
			"PermiButton": indexService.GetPermiButton,
		},
		DisableCache: true,
	})
